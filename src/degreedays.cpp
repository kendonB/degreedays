...
double degree_days_one(double t0, double t1,
  ...
  if (R_IsNA(tmin) || R_IsNA(tmax)){
    ...
    outt0 = degree_days_one(tmpt0, infty, tmin, tmax, 1.0);
    outt1 = degree_days_one(tmpt1, infty, tmin, tmax, 1.0);
    out = outt0 - outt1;
  } else if (t1 < tmin) {
...
NumericMatrix degree_days_band_daily(NumericVector t0, NumericVector t1,
  ...
  for (std::size_t i = 0; i < nrow; ++i) {
    for (std::size_t j = 0; j < ncol; ++j) {
      out(i, j) = degree_days_one(t0[j], t1[j], tmin[i], tmax[i], weights[i]);
    }
  }
  return out;
}
  ...
  void operator()(std::size_t begin, std::size_t end) {
    for (std::size_t i = begin; i < end; i++) {
      for(std::size_t j = 0; j < t0.size(); j++){
        // write to output matrix
        output(i,j) = degree_days_one(t0[j], t1[j], tmin[i], tmax[i], weights[i]);
      }
    }
  }
};
...
NumericMatrix degree_days_band_daily_par(NumericVector t0, NumericVector t1,
  ...
  // SquareRoot functor (pass input and output matrixes)
  DegDay degDay(t0, t1, tmin, tmax, weights, output);
